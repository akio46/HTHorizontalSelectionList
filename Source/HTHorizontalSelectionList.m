...
const CGFloat kHTHorizontalSelectionListTrimHeight = 3;
...
- (CGSize)collectionView:(UICollectionView *)collectionView
  ...
  sizeForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([self.dataSource respondsToSelector:@selector(selectionList:viewForItemWithIndex:)]) {
        ...
        CGFloat selectionIndicatorWidth = collectionView.frame.size.width / [self.dataSource numberOfItemsInSelectionList:self];
        return CGSizeMake(selectionIndicatorWidth, height);
    }
...
- (void)alignSelectionIndicatorWithCellWithIndexPath:(NSIndexPath *)indexPath {
    if (indexPath) {
        ...
        if (self.selectionIndicatorHorizontalPadding >= 0)
        self.selectionIndicatorBar.frame = CGRectMake(cellRect.origin.x + self.buttonInsets.left + kHTHorizontalSelectionListLabelCellInternalPadding/2 - self.selectionIndicatorHorizontalPadding,
            ...
            CGFloat selectionIndicatorWidth = self.contentView.frame.size.width / [self.dataSource numberOfItemsInSelectionList:self];
            self.selectionIndicatorBar.frame = CGRectMake(indexPath.row * selectionIndicatorWidth,
                                                          self.contentView.frame.size.height - self.selectionIndicatorHeight,
